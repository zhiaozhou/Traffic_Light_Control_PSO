...
sys.path.append("C:/DLR/tools") # Computer Specific.
...
global sss
if 'LATripInfo.xml' in os.listdir('C:/Users/Administrator/UTC-PSO/LA2'):
    sss = os.path.getctime('LATripInfo.xml')
else:
    sss = 0
###########################################################################
...
def update_networkfile(network,adict):
	...
	for ele in logic_list:
		if ele.attrib["id"] in list(adict.keys()):
...
def update_particle(currParticle, localBest, globalBest,currVelocity):
	...
	for i in list(currVelocity.keys()):
...
########################################################################################
def run_one_simulation(steps =5000):
	PORT = 8813
	"""
	...
	for step in range(steps):
		if step % 1000 == 0:
			print("step: {}/5000".format(step))
		traci.simulationStep()
		step +=1
	traci.close()
##########################################################################

def run_full_simulation(network, outputfile,sss,swarmSize, iterations=50):
	noofupdates =0
	updatesat =[]
	networkupdates =0
	"""
	...
	for i in range(iterations):
		...
		if i ==0:#for the first step only
			...
			for j in particle_list:
				...
				networkupdates+=1
				print ("#"+str(j))
				run_one_simulation(steps=5000) #finding fitness val.
				#os.system("""python randomTrips.py -n LAedited.net.xml -e 50 -r LA2.rou.xml  """)
				while not os.path.getmtime('LATripInfo.xml') > sss:
					time.sleep(1)
				if os.path.getmtime('LATripInfo.xml') > sss:
					sss = os.path.getmtime('LATripInfo.xml') 
...
###########################################################################################################################################################
run_full_simulation("LAedited.net.xml","LATripInfo.xml",sss,100,10)